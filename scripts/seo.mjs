#!/usr/bin/env node
// Deterministic SEO metadata + sitemap maintenance for the published static site.
// Visible page copy is always hand-authored; this script only owns the marker-delimited
// <head> block and docs/sitemap.xml.
...
function validateConfig(config) {
  ...
  for (const [index, page] of (config.pages || []).entries()) {
    ...
    if (typeof page.route !== 'string' || page.route.startsWith('/') || page.route.includes('..') ||
        page.route.includes('?') || page.route.includes('#') || (page.route && !page.route.endsWith('/'))) {
      errors.push(`${label}.route must be empty or a safe trailing-slash path relative to baseUrl`);
...
async function auditPublished(config, configDir, { requireFresh = true } = {}) {
  ...
  for (const page of config.pages) {
    ...
    if (requireFresh && generated !== html) errors.push(`${page.file}: generated SEO block is stale; run node scripts/seo.mjs --write`);
  ...
  if (requireFresh && currentSitemap && currentSitemap !== renderSitemap(config)) {
    errors.push(`${config.sitemap}: generated sitemap is stale; run node scripts/seo.mjs --write`);
  }
  return errors;
}
...
if (isCli) {
  ...
  if (write === check) {
    console.error('Usage: node scripts/seo.mjs (--write | --check) [--config path]');
    process.exitCode = 2;
  } else {
    try {
