...
its detail file. Rule of engagement: no mechanics/un-cheat/gate changes anywhere in
this backlog — views, CSS, input wiring, and staging only.
...
7. **S5 race mode takeover** (M) — menus collapse during play, big centered road,
   speed texture, docked/thumb steering + tap-to-steer, result overlay.
   [stage5.md #1-3]
8. **S8 sticky command bar** (S — do immediately after F1) + **spatial node map** (L).
   The command bar alone converts S8 from unplayable-loop to playable; the map is the
   big rock. [stage8.md #1-2]
...
10. **S3 board scale + boss staging + un-paginated shop + solve reveal** (M).
    [stage3.md #1-4]
11. **S7 board look + strings + triad sockets + verdict beats** (M+M). [stage7.md #1-4]

## Wave 3 — feel and finale
12. **S9 input (Space/tap-arena) + crossing marker + denser ring** (S+S+S) — cheapest
    whole-stage lift in the audit. [stage9.md #1-3]
13. **S10 memory grid + Phase-A reference + ceremony pass** (M+S+M) — the finale
    should land LAST so it inherits the mature feedback kit. [stage10.md #1-3]
