...
// ../../docs/games/metagame/stages/stage5/cards-protocol.js
var PROTOCOL_CARDS = [
  {
    id: "ACK",
    type: "Protocol",
...
// ../../docs/games/metagame/stages/stage5/cards-layer.js
var LAYER_CARDS = [
  {
    id: "CIPHER_LAYER",
    type: "Layer",
...
// ../../docs/games/metagame/stages/stage5/cards-daemon.js
var DAEMON_CARDS = [
  {
    id: "FORK_BOMB",
    type: "Daemon",
...
// ../../docs/games/metagame/stages/stage5/cards-recursion.js
var RECURSION_CARDS = [
  {
    id: "STACK_FRAME",
    type: "Recursion",
...
// ../../docs/games/metagame/stages/stage5/combat-rng.js
...
// ../../docs/games/metagame/stages/stage5/cards.js
...
// ../../docs/games/metagame/stages/stage5/combat-damage.js
...
// ../../docs/games/metagame/stages/stage5/combat-ctx.js
...
// ../../docs/games/metagame/stages/stage5/combat-piles.js
...
// ../../docs/games/metagame/stages/stage5/combat-enemy.js
...
// ../../docs/games/metagame/stages/stage5/combat-modes.js
var WINDOW_CAP = 5;
var WINDOW_FLOOR = 2;
...
// ../../docs/games/metagame/stages/stage5/combat.js
var HAND_SIZE = 5;
var START_ENERGY = 3;
...
// ../../docs/games/metagame/stages/stage5/enemies.js
var ENEMIES = {
  // ── Standard trash ──────────────────────────────────────────────────────────────────────────────
  "corrupt-packet": {
    id: "corrupt-packet",
...
// ../../docs/games/metagame/stages/stage5/relics.js
var RELICS = [
  {
    id: "handshake-token",
...
// ../../docs/games/metagame/stages/stage5/potions.js
var POTIONS = [
  {
    id: "hotfix",
    name: "Hotfix",
...
// ../../docs/games/metagame/stages/stage5/mapgen.js
var STANDARD_POOLS = {
...
// ../../docs/games/metagame/stages/stage5/card-upgrades.js
var UPGRADED_SUFFIX = "+";
var SPECS = {
...
// ../../docs/games/metagame/stages/stage5/superboss.js
...
// ../../docs/games/metagame/stages/stage5/run.js
...
// ../../docs/games/metagame/stages/stage5/events.js
...
// ../../docs/games/metagame/stages/stage5/testhook.js
...
function installStage5TestHook(api) {
  const {
    state,
    combatRun,
    runScore: runScore2,
...
function removeStage5TestHook() {
  if (window.__fvStage5) delete window.__fvStage5;
}

// ../../docs/games/metagame/stages/stage5/combat-persist.js
...
// ../../docs/games/metagame/stages/stage5/card-face.js
...
// ../../docs/games/metagame/stages/stage5/ui-combat.js
...
function eliteTelegraph(combat, run) {
  ...
  return `<p class="s5db-telegraph">take ≤5 damage this fight to earn the untouchable key ⚷</p>`;
}
function combatView(combat, run, opts = {}) {
  const el = document.createElement("div");
  el.className = "s5db-combat";
  const intent = currentIntent(combat);
  ...
  el.innerHTML = `
    ...
    <div class="s5db-battlefield">
      ${enemyPanel(combat.enemy, intent, combat)}
      ${playerPanel(combat.player)}
    </div>
    <div class="s5db-arena${arena ? "" : " is-empty"}">${arena}</div>
    ${inspectOverlay(combat, pending)}
    <div class="s5db-dock">
      <div class="s5db-dock-left">
        ${energyBlock(combat)}
        ${potionBelt(run)}
      </div>
      <div class="s5db-hand" aria-label="hand"></div>
      <div class="s5db-dock-right">
        ${pileChips(combat, run)}
        <button type="button" class="s5db-endturn" data-action="end-turn">end turn ▸</button>
      </div>
    </div>`;
  const hand = el.querySelector(".s5db-hand");
...
function arenaStrip(combat) {
  ...
  const ticker = lines.length ? `<div class="s5db-ticker">${lines.map((l) => `<p>${esc2(l)}</p>`).join("")}</div>` : "";
  const logChip = (combat.log || []).length ? `<button type="button" class="s5db-chip" data-log>log ▾</button>` : "";
  return `${ticker}${jammedRow(combat)}${logChip ? `<div class="s5db-arena-tools">${logChip}</div>` : ""}`;
}
...
function enemyPanel(enemy, intent, combat) {
  ...
  return `
    <section class="s5db-fighter s5db-enemy s5db-enemy--${enemy.tier || "standard"}">
      <div class="s5db-fighter-top">
        <span class="s5db-fighter-avatar" aria-hidden="true">${enemy.glyph || "👾"}</span>
        <span class="s5db-fighter-name">${esc2(enemy.name)}</span>
        ${tier ? `<span class="s5db-tier s5db-tier--${enemy.tier}">${tier}</span>` : ""}
      </div>
      ${bar(enemy.hp, enemy.maxHp, "enemy")}
      <div class="s5db-hp">HP ${enemy.hp} / ${enemy.maxHp}</div>
      <div class="s5db-meta">
        ${enemy.block ? `<span class="s5db-block">🛡 ${enemy.block}</span>` : ""}
        ${enemy.armor ? `<span class="s5db-armor">armor ${enemy.armor}</span>` : ""}
      </div>
      ${statusChips(enemy.statuses)}
      <div class="s5db-intent s5db-intent--${d.kind}" title="${esc2(intent?.label || "")}">
        <span class="s5db-intent-icon">${d.icon}</span>
        <span class="s5db-intent-num">${esc2(d.primary)}</span>
        <span class="s5db-intent-detail">${esc2(d.detail || intent?.label || "")}</span>
      </div>
...
function playerPanel(player) {
  return `
    <section class="s5db-fighter s5db-player">
      <div class="s5db-fighter-top">
        <span class="s5db-fighter-avatar" aria-hidden="true">💻</span>
        <span class="s5db-fighter-name">You</span>
      </div>
      ${bar(player.hp, player.maxHp, "player")}
      <div class="s5db-hp">HP ${player.hp} / ${player.maxHp}</div>
      <div class="s5db-meta">
        <span class="s5db-block">🛡 ${player.block}</span>
      </div>
...
function potionBelt(run) {
  ...
  const chips = potions.map((id, i) => {
    ...
    return `<button type="button" class="s5db-potion" data-potion="${i}" title="${esc2(p?.text || "")}">⚗ ${esc2(p?.name || id)}</button>`;
  }).join("");
  return `<div class="s5db-potions" aria-label="potion belt"><span class="s5db-potions-label">BELT:</span> ${chips}</div>`;
}
function jammedRow(combat) {
  if (!combat.jammed || !combat.jammed.length) return "";
  const chips = combat.jammed.map((id) => `<span class="s5db-card--jammed" title="Packet Loss — jammed">⛔ ${esc2(id)}</span>`).join("");
  return `<p class="s5db-jammed"><span class="s5db-jammed-label">JAMMED:</span> ${chips}</p>`;
}
function nextIntentTelegraph(enemy, combat) {
  ...
  return `<div class="s5db-intent-next" title="${esc2(next?.label || "")}">then ${d.icon} <strong>${esc2(d.primary)}</strong></div>`;
}
function energyBlock(combat) {
  const { energy, maxEnergy } = combat.player;
  const total = Math.max(maxEnergy, energy);
  let pips = "";
  for (let i = 0; i < total; i++) pips += `<span class="s5db-pip${i < energy ? " is-full" : ""}"></span>`;
  return `<div class="s5db-energy" aria-label="energy">
    <span class="s5db-pips" aria-hidden="true">${pips}</span>
    <span class="s5db-energy-num">${energy}/${maxEnergy}</span>
    ${combat.congestion && (combat.turn || 1) > 1 ? `<span class="s5db-window">⇄ ${combat.window}/${combat.windowCap}</span>` : ""}
  </div>`;
}
function pileChips(combat, run) {
  const chip = (kind, n, label) => `<button type="button" class="s5db-pilechip" data-pile="${kind}">${label} <b>${n}</b></button>`;
  const deckN = (run?.deck || []).length;
  return `<div class="s5db-pilechips">
    ${deckN ? `<button type="button" class="s5db-pilechip s5db-pilechip--deck" data-deck>deck <b>${deckN}</b></button>` : ""}
...
function inspectOverlay(combat, idx) {
  ...
  return `<div class="s5db-inspect" role="dialog" aria-label="inspect card">
    <div class="s5db-inspect-card s5db-card ${cardTypeClass(card)}" data-inspect="${idx}">${cardFaceInner(id)}</div>
    <p class="s5db-inspect-hint">${affordable ? "click again to play · Esc cancels" : "not enough energy"}</p>
  </div>`;
}
function statusChips(statuses) {
  const keys = Object.keys(statuses || {}).filter((k) => statuses[k]);
  if (!keys.length) return "";
  return `<div class="s5db-status">${keys.map((k) => `<span class="s5db-status-chip s5db-status-chip--${k}">${STATUS_LABEL[k] || k.toUpperCase()} ${statuses[k]}</span>`).join("")}</div>`;
}
function bar(value, max, who) {
  const pct = max > 0 ? Math.max(0, Math.min(100, Math.round(value / max * 100))) : 0;
  return `<div class="s5db-bar s5db-bar--${who}"><span style="width:${pct}%"></span></div>`;
}
function esc2(value) {
  return String(value).replace(/[&<>"]/g, (ch) => ({ "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;" })[ch]);
}

// ../../docs/games/metagame/stages/stage5/combat-fx.js
...
function applyCombatFx(node, fx) {
  ...
  if (fx.playerAttack) avatarAnim(player, "s5db-anim-lunge-l");
  if (fx.enemyAction === "attack") avatarAnim(enemy, "s5db-anim-lunge-r");
...
function flyCard({ rect, faceHTML }, arena) {
  ...
  clone2.className = "s5db-fly s5db-card";
...
// ../../docs/games/metagame/stages/stage5/ui-rewards.js
...
function rewardView(run) {
  ...
  el.innerHTML = `<h2>Signal recovered</h2>
    ${relic ? `<p class="s5db-relic-won">⬢ Relic acquired — <strong>${esc3(relic.name)}</strong>: ${esc3(relic.text)}</p>` : ""}
...
function bossRewardView(run) {
  ...
  row.replaceChildren(...offered.map((relic) => {
    ...
    b.innerHTML = `<strong>⬢ ${esc3(relic.name)}</strong><small class="s5db-card-text">${esc3(relic.text)}</small>`;
    return b;
  }));
  el.appendChild(row);
  el.insertAdjacentHTML(
...
function shopView(run) {
  ...
  potRow.replaceChildren(...potionOffers.map((id) => {
    ...
    b.innerHTML = `<strong>⚗ ${esc3(p?.name || id)}</strong><small class="s5db-card-text">${esc3(p?.text || "")}</small><span class="s5db-price">${POTION_COST} ✋</span>`;
    return b;
  }));
...
function eventView(run, event) {
  ...
  el.innerHTML = `
    ...
    <p>${esc3(event?.text || "")}</p>
    ${run.notice ? `<p class="s5db-hint">${esc3(run.notice)}</p>` : ""}`;
  const actions = document.createElement("div");
  actions.className = "s5db-hub-actions";
...
function keyTelegraph(run, keyId, text) {
  ...
  return `<p class="s5db-telegraph">${esc3(text)}</p>`;
}
...
// ../../docs/games/metagame/stages/stage5/combat-modals.js
...
// ../../docs/games/metagame/stages/stage5/combat-synergy.js
var KEYWORDS = {
...
// ../../docs/games/metagame/stages/stage5/combat-hover.js
function installCombatHover(root, getCombat) {
  let tip = null;
  const ensureTip = () => {
    if (!tip) {
      tip = document.createElement("div");
      tip.className = "s5db-tooltip";
      tip.hidden = true;
  ...
  root.addEventListener("mouseover", (event) => {
    ...
    const t = ensureTip();
    t.innerHTML = `<strong>${esc4(card?.name || id)}</strong><span>${esc4(card?.text || "")}</span>` + (kw ? `<em class="s5db-tooltip-keys">${kw}</em>` : "");
  ...
  root.addEventListener("mouseout", (event) => {
    ...
    if (to && to.closest && to.closest(".s5db-hand .s5db-card")) return;
    clear();
  });
}
function esc4(value) {
...
// ../../docs/games/metagame/stages/stage5/ui-map.js
var NODE_ICON = {
  combat: "⚔",
  elite: "☠",
  rest: "♨",
...
function ascensionPicker(asc, hasRun) {
  ...
  for (let n = 0; n <= asc.maxLevel; n++) {
    ...
    cells.push(locked ? `<span class="s5db-asc-cell is-locked" aria-disabled="true">${n}</span>` : `<button type="button" class="s5db-asc-cell${sel}${floorPinned}" data-ascension="${n}">${n}</button>`);
  }
...
function activeRules(level) {
  ...
  return `<ul class="s5db-modifiers" aria-label="active rules">${active.map((mod) => `<li>⚠ <strong>${esc5(mod.label)}</strong> — ${esc5(mod.desc)}</li>`).join("")}</ul>`;
}
function mapView(run) {
  ...
  const dots = Array.from({ length: total }, (_, i) => `<span class="s5db-act-dot${i + 1 < run.act ? " is-done" : ""}${i + 1 === run.act ? " is-here" : ""}"></span>`).join("");
  el.innerHTML = `<div class="s5db-map-head">
    ...
    ${run.notice ? `<div class="s5db-notice">${esc5(run.notice)}</div>` : ""}`;
  const grid = document.createElement("div");
...
function inventoryStrip(run) {
  ...
  const rItems = relics.length ? relics.map((r) => `<li><strong>⬢ ${esc5(r.name)}</strong> — ${esc5(r.text)}</li>`).join("") : `<li class="s5db-inv-none">No relics yet — clear elites and act bosses to earn them.</li>`;
  ...
  const keySection = keyRows ? `<div class="s5db-inv-keys"><h4>True-ending keys ${keys.length}/3</h4><ul>${keyRows}</ul></div>` : "";
...
function nodeChip(node, run, available, cleared) {
  ...
  chip.className = `s5db-node s5db-node--${node.type}` + (isAvailable ? " is-available" : "") + (isCurrent ? " is-current" : "") + (isCleared ? " is-cleared" : "") + (!isAvailable && !isCleared && !isCurrent ? " is-locked" : "");
  if (tag === "button") {
    chip.type = "button";
  ...
  chip.innerHTML = `<span class="s5db-node-icon">${NODE_ICON[node.type] || "?"}</span>
    <span class="s5db-node-type">${esc5(node.type)}</span>`;
  return chip;
}
...
function renderStage5({ host, state, save, orchestrator, onStageComplete }) {
  ...
  const combatRun = orchestrator?.save ? createRunState({ save: orchestrator.save, stageId: 5, slot: "combat", debounceMs: 0 }) : null;
  const ascension = orchestrator?.save ? createAscension({ save: orchestrator.save, stageId: 5, modifiers: ASCENSION_MODS }) : null;
