// ui-combat.js — Stage 5 combat screen (Slay-the-Spire layout: one viewport, no scroll).
//
...
export function combatView(combat, run, opts = {}) {
  ...
  // An EMPTY arena renders nothing visible (stage5 #6 — no dead box): the element stays in the flow
...
function bossBanner(combat) {
  ...
  return `
    <div class="s5db-boss-banner${locked ? " is-locked" : ""}">
      ...
      <p class="s5db-boss-rule">${esc(phaseRuleText(combat))}</p>
      ${locked
        ? `<p class="s5db-boss-mismatch">PROTOCOL MISMATCH — the handshake still lands, but Chapter 9 unread means a tougher fight (more HP).</p>
...
function arenaStrip(combat) {
  ...
  const ticker = lines.length ? `<div class="s5db-ticker">${lines.map((l) => `<p>${esc(l)}</p>`).join("")}</div>` : "";
  ...
  return `
    <section class="s5db-fighter s5db-enemy s5db-enemy--${enemy.tier || "standard"}">
      <div class="s5db-fighter-top">
        ...
        <span class="s5db-fighter-name">${esc(enemy.name)}</span>
      ...
      <div class="s5db-intent s5db-intent--${d.kind}" title="${esc(intent?.label || "")}">
        <span class="s5db-intent-icon">${d.icon}</span>
        <span class="s5db-intent-num">${esc(d.primary)}</span>
        <span class="s5db-intent-detail">${esc(d.detail || intent?.label || "")}</span>
      </div>
...
function potionBelt(run) {
  ...
  const chips = potions.map((id, i) => {
    ...
    return `<button type="button" class="s5db-potion" data-potion="${i}" title="${esc(p?.text || "")}">⚗ ${esc(p?.name || id)}</button>`;
  }).join("");
...
function jammedRow(combat) {
  ...
  const chips = combat.jammed.map((id) => `<span class="s5db-card--jammed" title="Packet Loss — jammed">⛔ ${esc(id)}</span>`).join("");
...
function nextIntentTelegraph(enemy, combat) {
  ...
  return `<div class="s5db-intent-next" title="${esc(next?.label || "")}">then ${d.icon} <strong>${esc(d.primary)}</strong></div>`;
}
...
// The raised inspect close-up (stage5 #2): the selected card enlarged above the hand. There is NO
...
function statusChips(statuses) {
  ...
  return `<div class="s5db-status">${keys
    .map((k) => `<span class="s5db-status-chip s5db-status-chip--${k}">${STATUS_LABEL[k] || k.toUpperCase()} ${statuses[k]}</span>`)
    .join("")}</div>`;
}
