...
{
  const save = { runs: {}, stageState: { 5: { run: { keep: true } } } };
  const run = createRun({ save, stageId: 5, slot: "combat", debounceMs: 0 });
  ...
  // The snapshot lands in stageState[5].combat WITHOUT clobbering stage5's own state.run.
  assert.ok(save.stageState[5].combat, "checkpoint wrote the combat slot");
  assert.equal(save.stageState[5].combat.runSeed, 9, "snapshot tagged with the run seed");
  assert.deepEqual(save.stageState[5].run, { keep: true }, "sibling state.run slot is untouched");
  ...
  run.reset();
  assert.equal(save.stageState[5].combat, undefined, "reset clears the combat slot");
  assert.equal(save.runs[5], 1, "reset bumps the run count");
  assert.deepEqual(save.stageState[5].run, { keep: true }, "reset leaves sibling slots intact");
  run.destroy();
}

console.log("stage5 combat-persist tests passed");
