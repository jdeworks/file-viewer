...
// ── Phase B re-witness goes through the token-verified gate, NOT a bare click ──────────────────────
{
  ...
  // Ignored: wrong stage / non-echo action.
  assert.deepEqual(onEchoAction({ state, detail: { stage: 9, action: "echo_genesis", token: echoTokenFor("genesis") } }), { ignored: true });
  assert.deepEqual(onEchoAction({ state, detail: { stage: 10, action: "memory_resolved" } }), { ignored: true });
  ...
  assert.equal(onEchoAction({ state, detail: { stage: 10, action: "echo_genesis" } }).spoofed, true);
  assert.equal(onEchoAction({ state, detail: { stage: 10, action: "echo_genesis", token: "bogus" } }).spoofed, true);
  ...
  const r = onEchoAction({ state, detail: { stage: 10, action: "echo_genesis", token: echoTokenFor("genesis") }, save: null });
...
{
  const state = defaultState({ now: 100 });
  const r = onEchoAction({ state, detail: { stage: 10, action: "echo_syntax", token: echoTokenFor("syntax") } });
  assert.equal(r.witnessed, true);
  assert.equal(r.rewitnessed, false);
  assert.equal(state.memories.syntax.echoWitnessed, true);
}

console.log("stage10 echo-gate tests passed");
