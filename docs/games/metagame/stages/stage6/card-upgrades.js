...
const SPECS = {
  ...
  // D2 DELAY
  WINDOWED_SEND: { text: "Deal 6. Deal 10 at the start of your next turn.", effect: (ctx) => { ctx.deal(6); ctx.queue(1, { deal: 10 }); } },
  RETRANSMIT: { text: "Deal 24 in 2 turns.", effect: (ctx) => ctx.queue(2, { deal: 24 }) },
  DELAYED_ACK: { text: "Gain 6 block. Gain 9 block at the start of your next turn.", effect: (ctx) => { ctx.block(6); ctx.queue(1, { block: 9 }); } },
  // D3 THROUGHPUT
