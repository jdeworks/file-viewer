...
// ── synthesis weaves all eight reflections + a stance-keyed closer ───────────────────────────────
{
  ...
  assert.equal(syn.parts.length, 8);
...
// ── capstone grid is personalized per stance choices ─────────────────────────────────────────────
{
  ...
  assert.equal(cap.tiles.length, 8);
  assert.equal(cap.integratedCount, 8);
  assert.equal(cap.resolvedCount, 8);
...
console.log("stage9 epilogue tests passed");
