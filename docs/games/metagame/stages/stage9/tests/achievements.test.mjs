...
// ── flawless compaction + all-traces conceded: clean recall + nothing to re-witness ──────────────
{
  ...
  // all 8 traces conceded → fragmentation auto-clears to core (no rewitness)
  ...
  assert.equal(store["stage9.flawless_compaction"], 1, "no compaction → flawless");
  assert.equal(store["stage9.all_traces_conceded"], 1, "all traces on record → conceded");
}
...
// ── a single mis-recall forfeits flawless; a manual re-witness forfeits all-traces ────────────────
{
  ...
  assert.equal(state.confront.completed, true);
  assert.equal(store["stage9.flawless_compaction"], undefined, "mis-recall → no flawless");
  assert.equal(store["stage9.all_traces_conceded"], undefined, "re-witness needed → no all-traces");
}
...
// ── route achievement on the final choice ─────────────────────────────────────────────────────────
{
  ...
  assert.equal(store["stage9.route_continue"], 1, "route badge for the chosen route");
}

console.log("stage9 achievements tests passed");
