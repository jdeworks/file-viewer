...
function renderGlyphs(card) {
  return `<span class="mg-stage9__glyphs" aria-hidden="true">${card.glyphs.map((g) =>
    `<span class="mg-stage9__glyph ${g.on ? "is-on" : "is-off"}" title="${escapeAttr(g.label)}">${g.letter}</span>`
  ).join("")}</span>`;
}
