...
import { stage9Markup } from "./markup.js";
...
export function renderStage9({ host, state, actions, achievements, bell, bts, viewer, save, onStageComplete }) {
  ...
  root.addEventListener("click", (event) => {
    ...
    if (event.target.closest(".s9-arena")) { doCross(); persistAndPaint(); }
  });
  ...
  function paintOverlays(seed, level, cfg) {
    ...
    fields.marker.classList.toggle("s9-marker--ready", intensity > 0.001);
  ...
  function repaint() {
    ...
    fields.arena.classList.toggle("s9-arena--unstable", unstable && !lock.unlocked); // #4 static/jitter border
