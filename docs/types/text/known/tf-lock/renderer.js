...
.tflockfile-table{width:100%;border-collapse:collapse;font-size:13px}
.tflockfile-table th{text-align:left;font-size:11px;text-transform:uppercase;letter-spacing:.04em;color:var(--fg-2,#888);padding:4px 8px;border-bottom:1px solid var(--border,#e0e0e0)}
.tflockfile-table td{padding:6px 8px;border-bottom:1px solid var(--border,#e0e0e0);vertical-align:top}
.tflockfile-name{font:13px/1.4 ui-monospace,monospace;font-weight:600}
.tflockfile-mono{font:12px/1.4 ui-monospace,monospace;color:var(--fg-2,#888)}
.tflockfile-badge{display:inline-block;padding:1px 6px;border-radius:4px;font-size:11px;font-weight:600;background:var(--bg-2,#f6f8fa);border:1px solid var(--border,#e0e0e0)}
`;
...
export function render(intake) {
  ...
  const rows = providers.map((p) => `<tr>
    <td><span class="tflockfile-name">${esc(p.shortAddr)}</span></td>
    <td><span class="tflockfile-badge">${esc(p.version)}</span></td>
    <td><span class="tflockfile-mono">${esc(p.constraints)}</span></td>
    <td><span class="tflockfile-mono">${p.hashCount} hash${p.hashCount !== 1 ? 'es' : ''}</span></td>
  </tr>`).join('');

  const tableHtml = providers.length
    ? `<table class="tflockfile-table"><thead><tr><th>Provider</th><th>Version</th><th>Constraints</th><th>Hashes</th></tr></thead><tbody>${rows}</tbody></table>`
