// Enhanced .eslintignore viewer: ESLint file exclusion patterns grouped by comment headers.
const esc = (s) => String(s == null ? '' : s).replace(/[&<>"]/g, (c) => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;' }[c]));

const CSS = `
.eslintignore-doc{padding:16px 18px;max-width:860px;margin:0 auto;font:14px/1.55 system-ui,sans-serif;color:var(--fg,#24292f)}
.eslintignore-badge{display:inline-block;padding:2px 9px;border-radius:10px;font-size:11px;font-weight:700;background:#4B32C3;color:#fff;vertical-align:middle;margin-right:8px}
.ei-title{font-size:18px;font-weight:700;margin:0 0 4px}
.ei-meta{margin:0 0 12px;display:flex;flex-wrap:wrap;gap:6px}
.ei-tag{display:inline-block;font-size:12px;padding:2px 8px;border-radius:10px;background:var(--bg-2,#f6f8fa);border:1px solid var(--border,#e0e0e0);color:var(--fg,#24292f)}
...
export function render(intake) {
  ...
  el.className = 'eslintignore-doc';
  el.innerHTML = '<style>' + CSS + '</style>'
    + '<div class="ei-title"><span class="eslintignore-badge">ESLint</span>.eslintignore</div>'
    + '<div class="ei-meta">'
    + '<span class="ei-tag">' + total + ' pattern' + (total === 1 ? '' : 's') + '</span>'
    + (negations > 0 ? '<span class="ei-tag">' + negations + ' negation' + (negations === 1 ? '' : 's') + '</span>' : '')
    + '</div>'
