...
const CSS = `
.sb-doc{padding:16px 18px;max-width:900px;margin:0 auto;font:14px/1.55 system-ui,sans-serif;color:var(--fg,#24292f)}
.badge-sb{display:inline-block;padding:2px 9px;border-radius:10px;font-size:11px;font-weight:700;background:#EF7C1A;color:#fff;vertical-align:middle;margin-right:8px}
.sb-title{font-size:18px;font-weight:700;margin:0 0 4px}
.sb-sub{font-size:12px;color:var(--fg-2,#888);margin:0 0 14px}
.sb-sec{margin:14px 0}
.sb-sec h3{font-size:12px;text-transform:uppercase;letter-spacing:.04em;color:var(--fg-2,#888);margin:0 0 7px}
.sb-card{border:1px solid var(--border,#e0e0e0);border-radius:8px;padding:10px 14px;background:var(--bg,#fff);margin:6px 0}
...
function renderMatch(matchEl, index) {
  ...
  if (bugEl) {
    ...
    if (pattern) parts.push(`<div class="sb-match-row"><span class="sb-label">Bug</span><span class="sb-val">${esc(pattern)}${category ? ` <span style="color:var(--fg-2,#888);font-weight:400">(${esc(category)})</span>` : ''}</span></div>`);
  }
  if (classEl) {
    const name = attr(classEl, 'name');
    const role = attr(classEl, 'role');
    if (name) parts.push(`<div class="sb-match-row"><span class="sb-label">Class</span><span class="sb-val">${esc(name)}${role ? ` <span style="color:var(--fg-2,#888)">[${esc(role)}]</span>` : ''}</span></div>`);
  }
  if (methodEl) {
    const name = attr(methodEl, 'name');
    const params = attr(methodEl, 'params');
    const returns = attr(methodEl, 'returns');
    if (name) parts.push(`<div class="sb-match-row"><span class="sb-label">Method</span><span class="sb-val">${esc(name)}${params || returns ? `(${esc(params)})${returns ? ':' + esc(returns) : ''}` : ''}</span></div>`);
  }
  if (fieldEl) {
    const name = attr(fieldEl, 'name');
    if (name) parts.push(`<div class="sb-match-row"><span class="sb-label">Field</span><span class="sb-val">${esc(name)}</span></div>`);
  }
  if (packageEl) {
    const name = attr(packageEl, 'name');
    if (name) parts.push(`<div class="sb-match-row"><span class="sb-label">Package</span><span class="sb-val">${esc(name)}</span></div>`);
  }
...
export async function render(intake) {
  ...
  if (doc.getElementsByTagName('parsererror').length) {
    ...
    d.className = 'sb-doc';
    d.innerHTML = `<style>${CSS}</style><p class="sb-err">Could not parse as XML.</p>`;
    return { parentNode: d };
  }
  ...
  const patternsHtml = bugPatterns.length
    ? `<div class="sb-sec"><h3>Bug Patterns (${bugPatterns.length})</h3>
<div class="sb-pills">
${bugPatterns.slice(0, SHOW_PILLS).map((p) => `<span class="sb-pill">${esc(p)}</span>`).join('')}
${bugPatterns.length > SHOW_PILLS ? `<span class="sb-pill" style="color:var(--fg-2,#888)">+${bugPatterns.length - SHOW_PILLS} more</span>` : ''}
</div></div>` : '';

  // Bug categories pills
  const categoriesHtml = bugCategories.length
    ? `<div class="sb-sec"><h3>Categories</h3>
<div class="sb-pills">
${bugCategories.map((c) => `<span class="sb-pill">${esc(c)}</span>`).join('')}
</div></div>` : '';

  // Class filters
  const classesHtml = classes.length
    ? `<div class="sb-sec"><h3>Class Filters (${classes.length})</h3>
<div class="sb-pills">
${classes.slice(0, SHOW_PILLS).map((c) => `<span class="sb-pill">${esc(c)}</span>`).join('')}
${classes.length > SHOW_PILLS ? `<span class="sb-pill" style="color:var(--fg-2,#888)">+${classes.length - SHOW_PILLS} more</span>` : ''}
</div></div>` : '';

  // Package filters
  const packagesHtml = packages.length
    ? `<div class="sb-sec"><h3>Package Filters</h3>
<div class="sb-pills">
${packages.map((p) => `<span class="sb-pill">${esc(p)}</span>`).join('')}
</div></div>` : '';

  // Match rules detail
  const SHOW_MATCHES = 20;
  const matchesHtml = matches.length
    ? `<div class="sb-sec"><h3>Match Rules (${matches.length})</h3>
${matches.slice(0, SHOW_MATCHES).map((m, i) => renderMatch(m, i)).join('')}
${matches.length > SHOW_MATCHES ? `<div class="sb-more">…and ${matches.length - SHOW_MATCHES} more rules</div>` : ''}
</div>` : '';

  const host = document.createElement('div');
  host.className = 'sb-doc';
  host.innerHTML = `<style>${CSS}</style>
<div class="sb-title"><span class="badge-sb">SpotBugs</span>${esc(filename)} <span style="font-size:13px;font-weight:400;color:var(--fg-2,#888)">${esc(filterType)}</span></div>
<div class="sb-sub">${esc(subParts)}</div>
${patternsHtml}
${categoriesHtml}
${classesHtml}
${packagesHtml}
