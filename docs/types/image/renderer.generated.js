...
function mountGifPlayer({ host, bytes, name, openBlob }) {
  ...
  async function splitFrames() {
    ...
    try {
      const blobs = await ensureFrameBlobs((d, t) => {
        splitBtn.textContent = `✂ Encoding ${d}/${t}…`;
      });
      if (destroyed) return;
  ...
  async function downloadAll() {
    ...
    try {
      const [JSZip, blobs] = await Promise.all([loadGlobal3(vendor3("jszip/jszip.min.js"), "JSZip"), ensureFrameBlobs()]);
      if (destroyed) return;
      const zip = new JSZip();
