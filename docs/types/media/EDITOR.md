...
- ✅ SHIPPED — **Drag-select region** — dragging on the audio waveform draws a highlighted
  region and pre-fills the existing ffmpeg editor Trim operation with start/end timestamps.
  It does not introduce a second trim path. — M
