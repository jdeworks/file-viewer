...
- Treat File Examples primarily as an extension catalogue, compatibility matrix source, and edge-case checklist rather than a trusted-valid sample source. Some downloaded files from that site are intentionally or accidentally broken; validate each one before promotion, and keep useful broken cases as explicit error-handling fixtures only when they improve recovery/warning behavior.
...
- Use real-world valid 3D samples such as `.example-files-internet/Arm_Chair.glb` to improve glTF/GLB fidelity. The mesh currently loads, but material/color information is lost compared with richer viewers; preserve base colors/materials/textures where feasible and surface unsupported material features clearly.
...
- Use the File Examples security demos as an internal security checklist rather than a public link target. Implement local defenses and warnings for MIME spoofing, double extensions, Unicode direction-control filename tricks, scriptable SVG, CSV formula injection, privacy-sensitive EXIF metadata, magic-byte mismatches, and suspicious active/executable content. Double-extension risk scoring should distinguish dangerous active endings such as `invoice.pdf.exe`, cautionary archive/document chains such as `document.pdf.zip`, legitimate multi-extension names such as `app.min.js`, and suspicious names inside archives. These should improve detection, metadata warnings, sanitization, and safe export behavior without linking users away for security guidance.
...
- Git repository analysis should cache expensive results for the current dropped repository/session. Branch clicks, file selection, metadata drawer opens, and view switches should reuse cached commit summaries, file maps, root detection, and computed stats unless a new folder/repository is loaded.
...
- For optional internet checks, clearly show what data would be sent, require explicit action, and handle offline mode. If an optional internet-backed check or external-resource opt-in is unavailable because the browser is offline or the user has not opted in, show a clear modal/status explaining that the core viewer remains local, identify the attempted feature, and offer retry/cancel without noisy console errors.
