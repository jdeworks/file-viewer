...
- `./scripts/check.sh --fast`: routine pre-push validation; regenerates `docs/asset-manifest.json`, runs LOC housekeeping, and scoped unit + core smoke. Three modes exist — `--fast` (per-push), bare `check.sh` (RELEASE gate, ~7 min: core smoke + representative samples of the heavy suites), `--exhaustive` (open-everything sweep ~13 min, on command — ALWAYS run it in a memory-capped container, never bare on the WSL2 host, or it can OOM-thrash the host to a hang; the script prints the `docker run` command). Add `--dry-run` to print a mode's selection.
...
Run `./scripts/check.sh --fast` before commits that affect app behavior, assets, vendored files, or tests; run the bare `./scripts/check.sh` (release gate) before a release/tag, and `--exhaustive` (in a memory-capped container — see the note above) when you need the full open-everything sweep. The smoke test depends on Playwright; if it is unavailable, run `cd tests && npm install && npx playwright install chromium`. Preserve the trust guarantee: runtime code should not introduce CDN, analytics, telemetry, or other off-origin requests.
