...
try {
  ...
  for (let index = 0; index < specs.length; index++) {
    ...
    await page.waitForFunction((core) => window.__fv?.state?.type?.id === 'emulatorjs'
      && window.EJS_core === undefined, spec.core);
  ...
  assert.deepEqual(ctx.offOrigin, [], 'six-core load makes no off-origin request');
