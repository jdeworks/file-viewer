...
- 2026-07-11: Request lifecycle implemented in source: accepted-intent activation guard, immutable preview snapshots, abort/cleanup registry with late-registration and identity deduplication, guarded renderer/context/iframe/archive commits, guarded raw-view creation, expected Monaco-cancellation suppression, side-by-side latest-request guard, and instance-scoped PPTX viewer disposal. Deterministic browser coverage now exercises package-toggle→Dockerfile, stale rejection, theme rerender ordering, current failure cleanup, raw-only replacement, side-by-side newer-wins, and side-by-side destroy-pending.
- 2026-07-11: Archive sibling navigation exposed one ownership boundary during the aggregate gate: the committed archive root must outlive the preview request that created it. The extractor is now transferred to the persistent archive-root lifecycle only after a current atomic commit, while iframe-originated open callbacks remain request-guarded. Focused `email-archives` coverage proves sibling reopening after entry replacement.
- Passing at this milestone: `node tests/preview-request-lifecycle.test.mjs`; `node tests/core-layout-regressions.mjs`; `node tests/smoke-area.mjs interactions`; `node tests/smoke-area.mjs tabular-office`; `node tests/smoke-area.mjs email-archives`; `node tests/mobile-renderer-layout.test.mjs`; syntax and diff checks; and the complete `./scripts/check.sh --fast`. The fast gate regenerated all derived assets, passed its full unit selection, aggregate zero-off-origin smoke, remote-resource privacy checks, and all 11 offline-readiness scenarios (with known-file, binary, and exhaustive Sokoban suites skipped exactly as documented for fast mode).
- Current step: 3 complete — transactional latest-request-wins preview rendering is ready for its milestone commit.
- Next resume action: add the permanent 47-row cap ledger/test and repair all `repair`/supplemental rows plus ragged CSV.
